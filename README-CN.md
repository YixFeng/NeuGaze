...
2. 将头部从中立位置向上、下、左或右移动。俯仰超过 `10°` 或偏航超过 `12°` 后，方向必须连续稳定 5 个有效处理帧才会锁定并高亮；两个方向同时超过阈值时，采用相对各自阈值偏移更大的方向。转头期间即使 `jawOpen` 短暂下降也不会提交。
3. 方向高亮后，把头部回到阈值内的中立死区，同时继续张嘴。系统在中立位置连续 3 个有效帧重新确认张嘴后，才允许闭嘴提交；已经锁定的高亮不会因回正而清除。
