...
- 点击"开始评估"
- 张嘴打开四方向机器人轮盘，保持张嘴并用注视选择方向，闭嘴确认
...
|----------|-----------------|----------------|-----------------|
| **张嘴** | `jawOpen > 0.4`，且下颌没有明显左右偏移 | `numlock` | 打开四方向机器人轮盘；保持张嘴，用注视选择，闭嘴确认 |
...
1. 张嘴触发 `numlock`，打开以屏幕中心为基准、半径为 400 的四方向轮盘。
2. 保持张嘴，将注视点移动到上、下、左或右区域。
3. 闭嘴确认；系统只输出选中的一个动作词条。
4. 如果闭嘴时没有有效选区，只输出显式取消行，不伪造动作。
...
当前机器人轮盘固定为半径 400 的四方向布局。头部俯仰、偏航和翻滚仍会被估计，
