...
当前 Ubuntu 24.04 Xorg 版本使用面部动作与注视轮盘输出机器人动作词条，为后续接入 GR00T Whole-Body Control 与 SONIC reference motion 做验证；Ubuntu 不发送游戏键位。
...
- 张嘴打开透明全屏四分区选择层，保持张嘴并用注视选择方向，闭嘴确认
...
|----------|-----------------|----------------|-----------------|
| **张嘴** | `jawOpen > 0.4`，且下颌没有明显左右偏移 | `numlock` | 打开透明全屏四分区选择层；保持张嘴，用注视选择，闭嘴确认 |
...
会点击鼠标或发送对应键盘按键。下颌左右移动、左右/双侧微笑及头部动作虽然仍可被
表情评估器识别，但当前没有配置机器人动作，因此不会输出词条。
...
1. 张嘴触发 `numlock`，打开覆盖整个主屏幕的透明选择层。
2. 屏幕由四条“角落到中心”的边界分成上、下、左、右四个等面积三角区；保持张嘴，将注视点移动到目标区域。整个屏幕都可用于选择，不再受中心圆形半径限制。
3. 闭嘴确认；系统只输出选中的一个动作词条。
4. 如果打开后尚未收到新的凝视点便闭嘴，或凝视点恰好位于中心，只输出显式取消行，不伪造动作。

选择层背景完全透明，不遮挡当前桌面；仅绘制低透明度分区边界。当前注视区显示低透明蓝色高亮，中文动作标签使用 `Noto Sans CJK SC` 字体和深色圆角底板。窗口保持置顶、点击穿透且不获取焦点。
...
当前机器人选择层固定为透明全屏四方向布局。配置不再接受 `radius`；如果布局不是
`fullscreen_cardinal`，程序会在启动时明确报错。头部俯仰、偏航和翻滚仍会被估计，
但没有绑定机器人动作，也不会映射到 W/S、A/D 或滚轮。
