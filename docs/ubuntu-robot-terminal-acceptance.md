...
每次均按真实身体序列执行：**张嘴打开透明全屏选择层 → 闭嘴 → 转头并锁定方向 → 回正并保持闭嘴 → 再次张嘴提交**。确认选择层覆盖整个主屏幕，桌面仍可见，只有对角分区线、四个中文动作标签和当前方向的低透明蓝色高亮；窗口不得抢焦点或拦截点击。当前 `numlock` 由 `jawOpen` 触发；选择只使用 MediaPipe 头姿角，不使用注视坐标。默认俯仰阈值为 `18°`、偏航阈值为 `12°`。第一次张嘴只负责打开轮盘，转头期间不需要持续张嘴；方向需连续稳定 5 个有效帧。回正并闭嘴连续 3 个有效帧后，再次张嘴连续 5 个有效帧立即提交，不需要最后闭嘴。人脸/blendshape 丢失不得提交，并会解除确认资格。前、后、左、右每个方向都完整重复该序列 5 次；每次等待对应输出并闭嘴后再开始下一次。
...
每次均按真实身体序列执行：**抬内眉打开透明全屏选择层 → 放松眉毛 → 转头并锁定方向 → 回正并保持未抬眉 → 再次抬眉提交**。方向稳定、回正确认、跟踪丢失保护和选择层视觉要求与张嘴轮盘相同；区别只在于开启和确认表情来自 `browInnerUp`。第一次抬眉只负责打开轮盘，转头期间不需要持续抬眉；回正并保持未抬眉连续 3 个有效帧后，再次抬眉连续 5 个有效帧立即提交，不需要最后放松。每个方向完整重复 5 次，每次等待对应输出并放松眉毛后再开始下一次。
...
1. 张嘴打开选择层后闭嘴，不锁定任何方向；头部保持中立并闭嘴 3 个有效帧，再次张嘴连续 5 个有效帧。
2. 抬眉打开选择层后放松眉毛，不锁定任何方向；头部保持中立且未抬眉 3 个有效帧，再次抬眉连续 5 个有效帧。
