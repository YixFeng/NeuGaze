...
让 NeuGaze 在 Ubuntu 24.04 Xorg 会话中完整运行，同时保留现有 Windows 支持。Ubuntu 版本覆盖 GUI、Orbbec Gemini 335 RGB 预览、校准、凝视鼠标、表情键鼠映射、轮盘和透明凝视层。安装阶段可在用户批准后使用 `sudo`，日常运行不得要求 root。
...
4. 凝视绝对和相对移动鼠标正常。
5. 左、右、中键、侧键和滚轮正常。
6. 键盘按下、保持、释放、组合键和安全释放正常。
7. 表情映射与轮盘选择正常。
8. 凝视层透明、置顶、输入穿透且不抢焦点。
9. ESC+Q 停止评估并释放所有资源。
10. 断开 Gemini 335 时显示原始错误并终止操作。
11. 日常运行过程不使用 root。
