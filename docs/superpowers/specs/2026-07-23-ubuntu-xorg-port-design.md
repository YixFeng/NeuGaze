...
安全按下只在目标键当前未按下时发送按下事件。安全释放只调用一次 `key_up_owned()`：未由 NeuGaze 持有时返回 false 且不发送事件；持有时发送一次 release 并同步，只有成功后才清除 ownership。release 或同步失败时保留 ownership，供调用方显式重试；不做全局状态 polling 或 timeout。退出路径同样只释放 NeuGaze 自己记录为按下的键和鼠标按钮，不释放外部物理输入。
