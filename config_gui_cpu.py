...
class ConfigWindow(QMainWindow):
    def __init__(self, parent=None):
        ...
        self.evaluation_stderr = bytearray()
    ...
    def start_evaluation(self):
        """Start or stop evaluation without blocking the Ubuntu GUI."""
        ...
        if process is not None:
            if process.state() != QProcess.NotRunning:
                ...
                return
            raise RuntimeError(
                "evaluation process finished without GUI cleanup"
            )
        ...
        if self.camera_platform == "linux":
            return self._start_linux_evaluation()
        ...
        try:
            return self.pipeline.start_evaluation()
        except Exception:
            formatted_traceback = traceback.format_exc()
            self._disable_camera_actions()
            QMessageBox.critical(
    ...
    def _start_linux_evaluation(self):
        if self.current_config_path is None:
            raise RuntimeError("Evaluation configuration path is not set")
        if self.pipeline is not None:
            raise RuntimeError(
                "Linux evaluation cannot start while a local pipeline exists"
            )
        ...
        try:
            ...
            self._show_linux_evaluation_error(traceback.format_exc())
            raise

        process = QProcess(self)
        self.evaluation_process = process
        self.evaluation_stop_requested = False
        self.evaluation_stderr.clear()
        process.readyReadStandardOutput.connect(
            self._read_evaluation_stdout
        )
        process.readyReadStandardError.connect(
            self._read_evaluation_stderr
        )
        process.finished.connect(self._finish_linux_evaluation)
        ...
        try:
            ...
            if not process.waitForStarted(5000):
                raise RuntimeError(
                    "Evaluation worker failed to start: "
            ...
            self.evaluation_stderr.clear()
            process.deleteLater()
            self._show_linux_evaluation_error(formatted_traceback)
            raise

        self.preview_label.clear()
        self.preview_label.setText(
            "Evaluation runs in a separate process. "
            "Configuration tabs remain available."
        )
        self.camera_confirm_btn.setEnabled(False)
        self.camera_change_btn.setEnabled(False)
        self.calibrate_btn.setEnabled(False)
    ...
    def stop_evaluation(self):
        ...
        process.terminate()
        QTimer.singleShot(
            EVALUATION_STOP_TIMEOUT_MS,
            lambda: self._kill_stuck_evaluation(process),
        )

    def _kill_stuck_evaluation(self, process):
        if self.evaluation_process is not process:
            return
        if process.state() == QProcess.NotRunning:
            return
        message = (
            "Evaluation worker did not stop within "
            f"{EVALUATION_STOP_TIMEOUT_MS / 1000:.1f} seconds; "
            "sending kill."
        )
    ...
    def _read_evaluation_stdout(self):
        process = self.evaluation_process
        if process is None:
            raise RuntimeError("Linux evaluation worker is not running")
        output = bytes(process.readAllStandardOutput())
        sys.stdout.buffer.write(output)
        sys.stdout.buffer.flush()

    def _read_evaluation_stderr(self):
        process = self.evaluation_process
        if process is None:
            raise RuntimeError("Linux evaluation worker is not running")
        output = bytes(process.readAllStandardError())
        self.evaluation_stderr.extend(output)
        sys.stderr.buffer.write(output)
        sys.stderr.buffer.flush()

    def _finish_linux_evaluation(self, exit_code, exit_status):
        process = self.evaluation_process
        if process is None:
            raise RuntimeError("Linux evaluation worker is not running")
        try:
    ...
    def _show_linux_evaluation_error(self, formatted_traceback):
